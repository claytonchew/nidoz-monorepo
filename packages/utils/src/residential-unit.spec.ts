...
describe("parseResidentialUnit", () => {
	describe("valid formats with hyphens", () => {
		it("should parse standard format A-01-01", () => {
			...
			expect(result).toEqual({ block: "A", floor: "01", number: "01" });
		});
	...
	describe("4 to 3A conversion", () => {
		it("should convert floor 04 to 3A", () => {
			...
			expect(result).toEqual({ block: "A", floor: "3A", number: "01" });
		});

		it("should convert number 04 to 3A", () => {
			const result = parseResidentialUnit("A-01-04");
			expect(result).toEqual({ block: "A", floor: "01", number: "3A" });
		});

		it("should convert both floor and number 04 to 3A", () => {
			const result = parseResidentialUnit("A-04-04");
			expect(result).toEqual({ block: "A", floor: "3A", number: "3A" });
		});

		it("should convert 14 to 13A", () => {
			const result = parseResidentialUnit("A-14-14");
			expect(result).toEqual({ block: "A", floor: "13A", number: "13A" });
		});

		it("should convert 24 to 23A", () => {
			const result = parseResidentialUnit("A-24-24");
			expect(result).toEqual({ block: "A", floor: "23A", number: "23A" });
		});
	...
	describe("valid formats without hyphens", () => {
		...
		it("should parse A0203 format", () => {
			...
			expect(result).toEqual({ block: "A", floor: "02", number: "03" });
		});
		...
		it("should parse A3A13A format (3A floor and 13A number)", () => {
			const result = parseResidentialUnit("A3A13A");
			expect(result).toEqual({ block: "A", floor: "3A", number: "13A" });
		});

		it("should parse A13A23A format", () => {
			const result = parseResidentialUnit("A13A23A");
			expect(result).toEqual({ block: "A", floor: "13A", number: "23A" });
		});
		...
		it("should parse AG01 format (Ground floor)", () => {
			...
			expect(result).toEqual({ block: "A", floor: "G", number: "01" });
		});

		it("should parse ALG02 format (Lower Ground floor)", () => {
			const result = parseResidentialUnit("ALG02");
			expect(result).toEqual({ block: "A", floor: "LG", number: "02" });
		});

		it("should parse ASB03 format (Sub-basement floor)", () => {
			const result = parseResidentialUnit("ASB03");
			expect(result).toEqual({ block: "A", floor: "SB", number: "03" });
		});

		it("should parse AB04 format (Basement floor with number 04->3A)", () => {
			const result = parseResidentialUnit("AB04");
			expect(result).toEqual({ block: "A", floor: "B", number: "3A" });
		});
	});
	...
	describe("invalid format errors", () => {
		...
		it("should throw error for invalid characters in floor/number", () => {
			expect(() => parseResidentialUnit("A-0X-01")).toThrow(
				/Invalid floor or number/,
			);
		});
	...
	describe("edge cases", () => {
		it("should handle floor 10", () => {
			...
			expect(result).toEqual({ block: "A", floor: "10", number: "01" });
		});

		it("should handle number 10", () => {
			const result = parseResidentialUnit("A-01-10");
			expect(result).toEqual({ block: "A", floor: "01", number: "10" });
		});

		it("should handle floor 34 (converts to 33A)", () => {
			const result = parseResidentialUnit("A-34-01");
			expect(result).toEqual({ block: "A", floor: "33A", number: "01" });
		});

		it("should handle different number lengths with hyphens", () => {
			const result = parseResidentialUnit("A-1-1");
			expect(result).toEqual({ block: "A", floor: "1", number: "1" });
		});

		it("should handle floor/number 4 (converts to 3A)", () => {
	...
	describe("special floors with hyphens", () => {
		...
		it("should parse B-LG-02 format (Lower Ground floor)", () => {
			...
			expect(result).toEqual({ block: "B", floor: "LG", number: "02" });
		});

		it("should parse C-SB-03 format (Sub-basement floor)", () => {
			const result = parseResidentialUnit("C-SB-03");
			expect(result).toEqual({ block: "C", floor: "SB", number: "03" });
		});

		it("should parse D-B-04 format (Basement floor with number 04->3A)", () => {
			const result = parseResidentialUnit("D-B-04");
			expect(result).toEqual({ block: "D", floor: "B", number: "3A" });
		});

		it("should parse A-G-3A format (Ground floor with 3A number)", () => {
			const result = parseResidentialUnit("A-G-3A");
			expect(result).toEqual({ block: "A", floor: "G", number: "3A" });
		});
